...
### امکانات (الهام‌گرفته از اپ «ببینیم» + متصل به بک‌اند اصلی هم‌فیلم)

> ✅ **نسخه ۱.۴.۰ — اتصال کامل به بک‌اند اصلی سایت** (`hamfilm-worker.ai-showcase-shir.workers.dev`):
> - پروتکل WebSocket دقیقاً مطابق بک‌اند: `play/pause/seek/speed` (به‌جای `control`)، `state/correct` برای همگام‌سازی،
>   `changeVideo`/`setLocalFile`، `reaction:{emoji,from}`، `peers.isLeader`، `chat.msg.name/ts/senderId`
> - **پخش هم‌زمان واقعی**: play/pause/seek با اصلاح خودکار وقتی اختلاف > ۱.۵ ثانیه است + سرعت پخش همگام
> - آواتارهای تصویری بامزه (a1..a10) مثل سایت — انتخاب در ساخت/ورود + نمایش در چت و اعضا
> - ورود با نام کاربری (نه ایمیل) مطابق بک‌اند + چک‌کردن اتاق قبل از ورود (کد ۶ کاراکتری + رمزدار)
> - تیکت‌ها با پیام‌های `user/admin/bot` (ربات خودکار پاسخ می‌دهد)
